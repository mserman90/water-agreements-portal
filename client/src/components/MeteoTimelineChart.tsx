...
export default function MeteoTimelineChart({
  ...
  if (loading && series.length === 0) {
    return (
      <div style={{ fontSize: 10, color: '#64748b', padding: '4px 8px' }}>
  ...
  if (error) {
    return (
      <div style={{ fontSize: 10, color: '#ef4444', padding: '4px 8px' }}>
    ...
    <div style={{ position: 'relative', width, userSelect: 'none' }}>
      ...
      >
        ...
        {agreementX !== null && (
          <>
            <line
              x1={agreementX} y1={PAD.top} x2={agreementX} y2={PAD.top + chartH}
              stroke="#f97316" strokeWidth={1.5} strokeDasharray="3,2" opacity={0.9}
            />
            <circle
              cx={agreementX}
        ...
        {hovered && (() => {
          ...
          if (!hp) return null;
          return (
            <circle cx={hp.x} cy={hp.y} r={3} fill="#fff" opacity={0.85} />
          );
        })()}

        {/* Year labels */}
        <text x={PAD.left} y={height - 1} fill="#475569" fontSize={8}>{series[0]?.year}</text>
        <text x={PAD.left + chartW} y={height - 1} fill="#475569" fontSize={8} textAnchor="end">{series[series.length - 1]?.year}</text>
      </svg>

      {/* Hover tooltip */}
      {hovered && (
        <div style={{
          position: 'absolute', top: 0, left: 0, fontSize: 10,
